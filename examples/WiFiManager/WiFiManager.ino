...
/*
 * EffectType: The output alphabet Γ of our Moore machine
 * 
 * Effects represent the outputs produced by the Moore machine based on the current state.
 * In Moore machine theory, outputs depend only on the current state (not on inputs).
 * Effects are executed by the main loop after being generated by the output function λ.
 */
enum EffectType {
...
/*
 * Effect: Output symbols from effect alphabet Γ
 * 
 * Effects carry information about what operations should be performed
 * based on the current state. They are generated by the output function λ
 * and executed by the main loop.
 */
struct Effect {
  EffectType type;                // Which effect this is
  AppMode previousMode;           // Previous mode for UI transitions
  AppMode currentMode;            // Current mode for UI updates
  
  Effect() : type(EFFECT_NONE), previousMode(MODE_INITIALIZING), currentMode(MODE_INITIALIZING) {}
  ...
  static Effect updateLEDs(AppMode mode) {
    Effect e;
  ...
  static Effect saveCredentials() {
    Effect e;
    e.type = EFFECT_SAVE_CREDENTIALS;
    return e;
  }
  
  static Effect startWiFiConnection() {
    Effect e;
    e.type = EFFECT_START_WIFI_CONNECTION;
    return e;
  }
  
  static Effect renderUI(AppMode mode) {
    Effect e;
  ...
  static Effect logConnectionSuccess() {
    Effect e;
    e.type = EFFECT_LOG_CONNECTION_SUCCESS;
    return e;
  }
  
  static Effect logConnectionLost() {
    Effect e;
...
void setup() {
  ...
  g_machine.addStateObserver(observeReconnectionRequests);
  g_machine.addStateObserver(observeModeChanges);
...
void executeEffect(const Effect& effect) {
  switch (effect.type) {
    ...
    case EFFECT_START_WIFI_CONNECTION: {
      ...
      // Signal that connection was started
      g_machine.step(Input::connectionStarted());
      break;
    }
...
void observeConnectedState(const AppState& oldState, const AppState& newState) {
  ...
  if (oldState.mode != MODE_CONNECTED && newState.mode == MODE_CONNECTED) {
    executeEffect(Effect::logConnectionSuccess());
    executeEffect(Effect::renderUI(newState.mode));
  }
}
...
void observeDisconnectedState(const AppState& oldState, const AppState& newState) {
  ...
  if (oldState.mode == MODE_CONNECTED && newState.mode == MODE_DISCONNECTED) {
    executeEffect(Effect::logConnectionLost());
    executeEffect(Effect::renderUI(newState.mode));
  }
}
...
void observeCredentialChanges(const AppState& oldState, const AppState& newState) {
  ...
  if (!oldState.credentialsChanged && newState.credentialsChanged) {
    executeEffect(Effect::saveCredentials());
    Serial.println("💾 Credentials saved successfully");
  }
}

// Observer: React to reconnection requests
void observeReconnectionRequests(const AppState& oldState, const AppState& newState) {
  // Trigger when shouldReconnect flag is set
  if (!oldState.shouldReconnect && newState.shouldReconnect) {
    executeEffect(Effect::startWiFiConnection());
  }
}

// Observer: React to mode changes for UI updates
void observeModeChanges(const AppState& oldState, const AppState& newState) {
  // Update UI when mode changes (except for connection/disconnection which have specific observers)
  if (newState.mode != oldState.mode && 
      !(oldState.mode != MODE_CONNECTED && newState.mode == MODE_CONNECTED) &&
      !(oldState.mode == MODE_CONNECTED && newState.mode == MODE_DISCONNECTED)) {
    executeEffect(Effect::renderUI(newState.mode));
  }
}
...
void loop() {
  ...
  // 3. Read events from environment (user input, hardware status)
